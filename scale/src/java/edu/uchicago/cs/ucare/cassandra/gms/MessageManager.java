...
public class MessageManager {
    ...
    private Map<InetAddress, List> sentMessagesPerHost = new HashMap<InetAddress, List>();
    private Map<InetAddress, List> receivedMessagesPerHost = new HashMap<InetAddress, List>();

    private Map<InetAddress, Integer> gossipRoundsPerHost = new ConcurrentHashMap<InetAddress, Integer>();

    private Map<InetAddress, Integer> receivedPerHost = new ConcurrentHashMap<InetAddress, Integer>();
    ...
    private void removeMessageQueue(InetAddress host, Map<InetAddress, List> map) {
        List q = map.get(host);
        if (q != null && q.size() == 0) {
            synchronized (map) {
                map.remove(host);
    ...
    private int getSizeOf(InetAddress host, Map<InetAddress, List> map) {
    ...
    public GossipRound pollNextSentMessage(InetAddress host, String basePath) {
        ...
        if (nextMessageId == null)
            return null;
        String messageFileName = MessageUtils.buildSentGossipFilePathForRound(
                new GossipRound(nextMessageId), basePath, host);
        // now load
        ...
        try {
            ...
            logger.error("@Cesar: Skipped a message < " + nextMessageId
                    + ">since cannot load", e);
            return null;
        } finally {
            if (strm != null) {
    ...
    public ReceivedMessage pollNextReceivedMessage(InetAddress host,
        ...
        if (nextMessageId == null)
            return null;
        String messageFileName = MessageUtils
                .buildReceivedMessageFilePathForRound(new ReceivedMessage(
                        nextMessageId), basePath, host);
        // now load
        FileInputStream strm = null;
        ObjectInputStream in = null;
        try {
    ...
    private Integer pollNext(InetAddress host, Map<InetAddress, List> map) {
    ...
    public void addSentMessage(InetAddress host, int message) {
        addToQueue(host, message, sentMessagesPerHost);
    }

    public void addReceivedMessage(InetAddress host, int message) {
        addToQueue(host, message, receivedMessagesPerHost);
    }

    private void addToQueue(InetAddress host, int message,
    ...
    public void saveRoundToFile(final GossipRound round, final String basePath,
            final InetAddress id) {
        new Thread() {
            @Override
            public void run() {
                String fileName = MessageUtils.buildSentGossipFilePathForRound(
                        round, basePath, id);
                String mapFileName = MessageUtils
                        .buildSentGossipFilePathForMap(basePath, id);
                PrintWriter pr = null;
    ...
    public void saveMessageToFile(final ReceivedMessage message,
            final String basePath, final InetAddress id) {
        String fileName = MessageUtils.buildReceivedMessageFilePathForRound(
                message, basePath, id);
        String mapFileName = MessageUtils.buildReceivedMessageFilePathForMap(
                basePath, id);
        PrintWriter pr = null;
        ObjectOutputStream out = null;
        FileOutputStream fopt = null;
    ...
    private void loadRoundsFromFile(String basePath, InetAddress id) {
        String mapFileName = MessageUtils.buildSentGossipFilePathForMap(
                basePath, id);
        ...
        try {
            ...
            brdr.close();
            logger.info("@Cesar: <" + getSentMessageQueueSize(id)
                    + "> gossip rounds loaded " + "for host <" + id
                    + "> from <" + mapFileName + ">");
        } catch (Exception ioe) {
            logger.error("@Cesar: Exception while loading <" + mapFileName
                    + ">", ioe);
        } finally {
            try {
                if (brdr != null)
    ...
    private void loadMessagesFromFile(String basePath, InetAddress id) {
        ...
        try {
            ...
            brdr.close();
            logger.info("@Cesar: <" + getReceivedMessageQueueSize(id)
                    + "> messages loaded " + "for host <" + id + "> from <"
                    + mapFileName + ">");
        } catch (Exception ioe) {
            logger.error("@Cesar: Exception while loading <" + mapFileName
                    + ">", ioe);
    ...
    public void loadSentGossipRounds(String basePath,
            List<InetAddress> addressList) {
        sentMessagesPerHost.clear();
        for (InetAddress address : addressList)
            loadRoundsFromFile(basePath, address);
    }

    public void loadReceivedMessages(String basePath,
            List<InetAddress> addressList) {
        receivedMessagesPerHost.clear();
        for (InetAddress address : addressList)
            loadMessagesFromFile(basePath, address);
    }
